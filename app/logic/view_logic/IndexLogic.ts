    ...
    private setRoomList(list: any[]) {
        ...
        for (let x = 0, l = list.length; x < l; x++) {
            ...
            if (list[x]['activity']) {
                ...
                startDate = new Date(Date.parse(activity['startDate'].replace(/-/g, "/")));
                endDate = new Date(Date.parse(activity['endDate'].replace(/-/g, "/")));
            }
            if (!list[x]['src']) continue;
            html += `<li>
                <a href="javascript:void(0);" class="room-info" data-id="${list[x]['id']}">
                    <img class="lazy"  data-src="${((today > startDate) && (today < endDate)) ? Config.imgBase + activity['banner'] : Config.imgBase + list[x]['src']}" alt="">
                </a>
                <div class="item-msg">
                    <div class="left">
                        <h3 class="font-clip">${((today > startDate) && (today < endDate)) ? activity['title'] : list[x]['title']}</h3>
                        <span class="flex">
